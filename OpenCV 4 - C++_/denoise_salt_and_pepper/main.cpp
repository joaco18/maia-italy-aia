...
namespace
{
    ...
    void salPeperDenoiseCallback(int, void*)
    {
...
int main()
{
    ...
    cv::createTrackbar(
        "Noise level", win_name, &salt_pepper_perc, 100, salPeperDenoiseCallback
    );
    cv::createTrackbar(
        "Filter size", win_name, &median_filter_size, 50, salPeperDenoiseCallback
    );
    img_corrupted = img.clone();
    salPeperDenoiseCallback(0,0);
    cv::waitKey(0);
