...
namespace
{
	...
	cv::Mat frameProcessor(const cv::Mat& img)
	{
		...
		cv::Mat binarized_v;
		cv::threshold(
			hsv_channels[2], binarized_v, 0, 255,
		...
		cv::findContours(
			binarized_v, all_objects, cv::RETR_LIST, cv::CHAIN_APPROX_NONE
		);

		// STEP3: get circular objects only = filter by shape
		...
		for (int i = 0; i < all_objects.size(); i++)
		{
			...
			if (A >= A_min && A <= A_max)
			{
				...
				if (C >= C_min)
				{
					// third criterion: color 
					double avg_S = avgValInContour(
						hsv_channels[1], all_objects[i]
					);
					if (avg_S >= S_min)
						cv::drawContours(
							out_img, all_objects, i, cv::Scalar(0, 255, 255),
							2, cv::LINE_AA
						);
				}
			}
...
int main()
{
	aia::processVideoStream(
		std::string(EXAMPLE_IMAGES_PATH) + "/traffic_light_4.mp4",
		frameProcessor, "", true, 0
	);
	return EXIT_SUCCESS;
}